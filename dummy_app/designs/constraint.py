...
def individual_scenario_constraints(cluster:Any, builder:Any , V_nodes:list, list_of_agents:dict):
    ...
    MANDATORY_WAIT_TIME = builder.coverage_time
    wait_energy_consumption = builder.average_coverage_energy
    bridge_nodes = set(cluster.virtual_nodes.values()) 
    ...
    for k in agents: 
        for hub in bridge_nodes: 
            virtual_copies = [v_node for v_node in cluster.virtual_nodes if cluster.virtual_nodes[v_node] == hub] 
            model += pl.lpSum(cluster.visit[reverse_nodes[v_node],k] for v_node in virtual_copies) >= 1
    ...
    for j in NODES: 
        for k1 in agents: 
            for k2 in agents: 
                ...
                model += cluster.t[j,k2] >= (cluster.t[j,k1] + MANDATORY_WAIT_TIME) - M * (1 - cluster.precedes[j,k1,k2]) 

                model += cluster.t[j,k1] >= (cluster.t[j,k2] + MANDATORY_WAIT_TIME) - M * (cluster.precedes[j,k1,k2])
