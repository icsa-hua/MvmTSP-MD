...
def constraint_2(cluster:Any, builder:Any , V_nodes:list, list_of_agents:dict):
    ...
    for k, v in list_of_agents.items(): 
        ...
        cluster.problem.addConstraint(
            ...
            constraint = pl.lpSum(cluster.t[i, depot_ind, v, t] for i in V_nodes if i != depot_ind for t in cluster.timeframe[:-(cluster.tr_times[(i,depot_ind)]+1)]) >= 1, 
        )
...
def constraint_8(cluster:Any, builder:Any , V_nodes:list, list_of_agents:dict):
    ...
    #         for k, v in list_of_agents.items() : 
    #             cluster.problem += pl.lpSum(cluster.t[i,j,v,t] for t in cluster.timeframe) <= 1 
    #             # cluster.problem += pl.lpSum(cluster.t[i,j,v,t] for t in cluster.timeframe) <= d *cluster.x[i,j,v]
        ...
        for i in V_nodes:
            for j in V_nodes:
                ...
                cluster.problem.addConstraint(
                    ...
                    constraint=cluster.e[j,v] >= cluster.e[i,v] - energy_cost - M * (1-cluster.x[i,j,v])
                )

                cluster.problem.addConstraint(
                    name=f"No_travel_if_low_energy_{i}_{j}_for_{k}",
                    constraint=cluster.e[i,v] >= energy_cost - M *(1 - cluster.x[i, j, v])
                )

            cluster.problem.addConstraint(
                name=f"Enough_energy_to_return_to_depot_from_{i}_for_{k}",
                constraint=cluster.e[i,v] >= builder.move_energy[cluster.nodes_dict[i]][cluster.depot_id] * cluster.x[i,depot_ind,v]
            )
        ...
        for t in cluster.timeframe: 
            for i in V_nodes: 
                ...
                cluster.problem.addConstraint(
                    ...
                    constraint=cluster.e[i, v] >= cluster.e[i, v] - cov_cost * cluster.wait[v, t],
                )

                # Optional: ensure energy is enough before waiting
                cluster.problem.addConstraint(
                    name=f"No_wait_if_low_energy_{i}_at_{t}_for_{k}",
                    constraint=cluster.e[i, v] >= cov_cost * cluster.wait[v, t],
                )
