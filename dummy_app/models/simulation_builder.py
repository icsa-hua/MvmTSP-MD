...
class Builder(MVMTSPConfig):
    ...
    def create_solution(self, V_nodes, nodes_dict):
        ...
        for agent_name, agent_id in list_of_agents.items():
            node_values = [v for k, v in nodes_dict.items()]
            node_values.remove(self.depots_for_agents[agent_id])
            step = self.timeFrame_per_cluster[0]
            next_node = 0 
            current_node = 0 
            while step in self.timeFrame_per_cluster: 
                if step == self.timeFrame_per_cluster[0]:
                    self.paths[agent_id].extend((self.depots_for_agents[agent_id],step))
                    current_node = self.depots_for_agents[agent_id]
                    next_node = random.choice(node_values) 
                    node_values.remove(next_node)
                    step += 1 
                    continue
                elif step >= self.timeFrame_per_cluster[-1]:
                    self.paths[agent_id].extend((self.depots_for_agents[agent_id]),step)
                    break
            ...
            if self.paths[agent_id][-1] != self.depots_for_agents[agent_id]:
                self.paths[agent_id].extend((self.depots_for_agents[agent_id],step))
    ...
    def set_constraints_for_multi_agent(self, V_nodes:List[int], nodes_dict:Dict[int, int], R_points:List[int]): 
        ...
        if "const_3" in self.constraints: #NOTE FInalized. Has to be equal to 1 because otherwise it 
            try: 
                for k, v in list_of_agents.items():
                    for j in V_nodes:
                        if j != depot_ind:
                ...
                for k2, v2 in list_of_agents.items() : 
                    if k1 != k2 : 
                        for step in self.timeFrame_per_cluster: 
                            ...
                            self.problem += pl.lpSum(self.t[i, j, v1, step] - self.t[i,j, v2, step] for i in V_nodes for j in V_nodes if i!= j) <= 0, f"Agent_unique_paths_for_{k1}_and_{k2}_at_time_{step}"
        ...
        if "const_15" in self.constraints:
            M = len(V_nodes)+ len(self.bridge_nodes) -1 
            for k, v in list_of_agents.items():
                for t in self.timeFrame_per_cluster:
        ...
        if "const_17" in self.constraints:
            for k in self.agents:
                for t in self.timeFrame_per_cluster:
                    self.problem += self.busy[k, t] + self.wait[k, t] <= 1
